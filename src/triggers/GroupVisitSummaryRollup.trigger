trigger GroupVisitSummaryRollup on Volunteer_Visit__c (after delete, after insert, after undelete, 
after update) {
	...
	if(Trigger.isInsert || Trigger.isUnDelete) {
		...
		if(GrpIDs.size()>0) {RollupToGroup.DoVisit(GrpIDs);}	
	}	
	...
	if( Trigger.isUpdate ) {
		...
		for (integer i = 0; i<Trigger.new.size(); i++) {
				...
				if((Trigger.old[i].Status__c == 'Completed' || Trigger.new[i].Status__c == 'Completed'||Trigger.old[i].Status__c == 'Completed - Send Thanks' || Trigger.new[i].Status__c == 'Completed - Send Thanks') && (Trigger.new[i].Volunteer_Type__c == 'Group' ||Trigger.new[i].Volunteer_Type__c == 'Family' ))
					{if(Trigger.new[i].Org_Group__c<>NULL) {GrpIDs.add(Trigger.new[i].Org_Group__c);}}
